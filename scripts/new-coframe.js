...
write(path.resolve(base,'coframe.manifest.json'), JSON.stringify(manifest,null,2));
write(path.resolve(base,'declaration.md'), decl);
write(path.resolve(base,'changes.md'), changes);
write(path.resolve(base,'constructor/CONSTRUCTOR-PROMPT.md'), prompt);
try{ const schemaPath = path.resolve(root,'schema/context.manifest.json'); const schema = loadJson(schemaPath); schema.coframes = schema.coframes || {}; schema.coframes[name] = 'coframes/' + name + '/coframe.manifest.json'; write(schemaPath, JSON.stringify(schema,null,2)); console.log('Registered in schema.context.manifest.json'); }catch(e){ console.warn('WARN: schema update failed:', e.message); }
